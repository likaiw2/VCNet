...
#这个第一个pathlist不用了，因为太蠢了，要一个个手输20遍
pathlist = [1199, 2799, 4399, 7199, 9199,
            11999, 12799, 14399, 15999, 17599,
            19199, 20399, 21999, 23199, 24799,
            25199, 26399, 27199, 15199, 27999]

#这个是真正的pathlist
pathlist = [(n+1)*3500-1 for n in range(20)]
# print(pathlist)
...
def saveRawFile10(dataSavePath, cur_step, res, t, l, volume):
    ...
    fileName = '%s%s_%.4d.raw' % (dataSavePath, res, cur_step)
...
def predict(p):
    ...
    weights_path = "%s%s%s%s" % (sourceWeightsPath, "DCGAN_", p, ".pth")
    dataSave_path = "%sDCGAN_%s/" % (dataSavePath, p)
    ...
    with torch.no_grad():
        ...
        for ct, mri, index in dataloader:
            ...
            saveRawFile10(dataSave_path, index+1, 'DCGAN_pred_norm_mr', index, '', fake[0, 0, :, :, :])
            # saveRawFile10(dataSavePath, index+1, 'truth_mr', index, '', mri[0, 0, :, :, :])
            t = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
            print(t, "  ", index+1, " saved")


# output all prediction of 20 selected epochs
for p in pathlist:
    # print(weights_path)
    print("----predicting:DCGAN_%s----" % p)
    t_start = time.time()
    t_start_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
    predict(p)
    t_end = time.time()
    t_end_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
    dt=t_end-t_start
    total_time = str(datetime.timedelta(seconds=dt))
    per_image = str(datetime.timedelta(seconds=dt/30))
    print(f"DCGAN_{p}:  \nstart:{t_start_str}    end:{t_end_str}    \ntime_total:{total_time}    time_each:{per_image}")
    print()
